...
HTML_PARTIAL = File.read(Path[__DIR__] / "_header.html")
CSS_PARTIAL = File.read(Path[__DIR__] / "_style.css")
...
File.open(OUTPUT_DIR / "css/style.css", "a") do |style|
  style.puts CSS_PARTIAL
  style.flush
end
puts CSS_PARTIAL
...
def process!(filename : String)
  ...
  content = head + "<body>" + HTML_PARTIAL + tail
